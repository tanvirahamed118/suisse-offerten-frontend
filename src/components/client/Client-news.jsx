...
function ClientNews({ jobId, sellerId }) {
  ...
  const sellerData = useMemo(() => {
    if (sellerId) {
      return data?.filter(
        (item) => item.jobId === jobId && item?.sellerId === sellerId
      );
    }
    return [];
  }, [data, jobId, sellerId]);
  ...
  useEffect(() => {
    if (data) {
      const filteredMessages = data
        .filter((item) => item.jobId === jobId && item.sellerId === sellerId)
        .flatMap((item) => [
