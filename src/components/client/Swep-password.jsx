...
import { useUpdatePasswordClientMutation } from "../../redux/rtk/features/auth/client/authApi";

function SwepPassword({ id }) {
  const { t } = useTranslation();
  const [client, setClient] = useState({
    password: "",
  });
  const [updatePasswordClient, { data, isLoading, isError, isSuccess, error }] =
  ...
  const handleSubmit = (e) => {
    ...
    if (password !== confirmPass) {
      ...
      updatePasswordClient({ client, id });
    }
  };
...
SwepPassword.propTypes = {
  id: PropTypes.string,
};

export default SwepPassword;
